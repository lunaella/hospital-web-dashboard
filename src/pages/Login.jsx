...
// This page used to be a fixed 1440x1024 canvas scaled to fit the window
// (via ScaleToFit). That guaranteed pixel-perfect Figma positions at any
// window size, but a fixed-aspect-ratio canvas can't simultaneously
// guarantee "no scroll" and "no empty gutters" for every window shape —
// shrinking the whole canvas to fit a short-but-wide window (contain-fit)
// eliminates the scrollbar but leaves large empty bars on the sides.
// The only way to get both is to stop scaling a fixed canvas and make the
// page genuinely fluid: min-h-screen instead of a fixed height, a flex
// row/column split instead of absolute left/top coordinates, so the layout
// itself adapts to the viewport rather than a miniature of it being
  ...
  return (
    ...
    >
      <div className="relative z-10 flex-1 w-full max-w-[1320px] mx-auto flex flex-col lg:flex-row items-center justify-center gap-12 lg:gap-16 px-6 sm:px-10 lg:px-12 py-14">
        {/* Logo + huge bold headline + description + chips — one normal-flow
            flex column, so however many lines any of it wraps to, everything
            below always sits at the right distance instead of relying on a
            hardcoded offset that can go stale and overlap. */}
