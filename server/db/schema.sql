...
-- Models the entities backing the admin dashboard: donors, blood inventory,
-- requests/broadcasts, appointments, fulfillment history, admin accounts,
-- sessions, and system health metrics.
...
-- One row per blood type; units_available drives the Stock Criticality card.
CREATE TABLE blood_inventory (
  blood_type        blood_type PRIMARY KEY,
...
-- Fulfillment Log, and Reports KPIs all derive from this table).
...
INSERT INTO blood_inventory (blood_type, units_available, critical_threshold, low_threshold) VALUES
  ('O-', 12, 15, 25),
  ('AB-', 12, 10, 15),
  ('A+', 12, 10, 15),
  ('O+', 12, 10, 15);
...
INSERT INTO blood_requests (request_code, blood_type, priority, ward, units_needed, units_fulfilled, status, created_at, resolved_at) VALUES
  ('REQ-9012', 'O-', 'EMERGENCY', 'ICU-4', 10, 4, 'PARTIALLY_FULFILLED', now() - INTERVAL '12 minutes', NULL),
  ('REQ-8843', 'A+', 'EMERGENCY', 'ER-A', 3, 2, 'PARTIALLY_FULFILLED', now() - INTERVAL '28 minutes', NULL),
  ('REQ-9104', 'B-', 'URGENT', 'Surgery-B', 5, 1, 'PARTIALLY_FULFILLED', now() - INTERVAL '45 minutes', NULL),
  ('REQ-8756', 'AB+', 'URGENT', 'General-2', 5, 5, 'FULFILLED', now() - INTERVAL '65 minutes', now() - INTERVAL '12 minutes'),
  ('REQ-9211', 'O+', 'NORMAL', 'Dialysis', 15, 8, 'PARTIALLY_FULFILLED', now() - INTERVAL '35 minutes', NULL);
...
SELECT id, date_trunc('day', now()) + INTERVAL '9 hours 30 minutes', 'confirmed'::appointment_status FROM donors WHERE donor_code = 'D-8821'
UNION ALL
SELECT id, date_trunc('day', now()) + INTERVAL '10 hours 15 minutes', 'pending'::appointment_status FROM donors WHERE donor_code = 'D-9012'
UNION ALL
SELECT id, date_trunc('day', now()) + INTERVAL '11 hours', 'pending'::appointment_status FROM donors WHERE donor_code = 'D-3321';

INSERT INTO system_health_snapshots (min_heap_latency_ms, db_sync_status, overall_status) VALUES
  (12, 'Real-time', 'STABLE');

INSERT INTO donor_arrivals (donor_id, arrived_at)
SELECT id, now() - INTERVAL '2 minutes' FROM donors WHERE donor_code = 'D-8821'
UNION ALL
SELECT id, now() - INTERVAL '8 minutes' FROM donors WHERE donor_code = 'D-9012'
UNION ALL
SELECT id, now() - INTERVAL '14 minutes' FROM donors WHERE donor_code = 'D-7742'
UNION ALL
SELECT id, now() - INTERVAL '21 minutes' FROM donors WHERE donor_code = 'D-3321';
